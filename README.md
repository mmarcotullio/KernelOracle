...
------------------------------------------------------------------------
We aim to improve performance by replacing the baseline LSTM model with a temporal concolutional neural network (TCN). Additionally, we will be training on more diverse workloads, using ftrace to collect (real) kernel scheduling data (hackbecnh, sysbench, custom cpu-bound loops, I/O workloads).  
